...
class TweetCleaner:
    ...
    def clean(self, tweet):
        regex = {k: re.compile(self.REGEX_DICT[k]) for k, v in self.REGEX_DICT.items()}

        for key, reg in regex.items():
            tweet = reg.sub(lambda m: " <" + key + "> ", tweet)
        ...
        tweet = unicodedata.normalize('NFKD', tweet).encode('ascii', errors='ignore').decode('utf8', errors='ignore')
        tweet = re.sub(r'\b([b-hB-Hj-zJ-Z] )', ' ', tweet)
        tweet = re.sub(r'( [b-hB-Hj-zJ-Z])\b', ' ', tweet)
